...
ArrayToGraph.prototype.CreatechartPower = function (data, local) {
    var points = [];
    for (var i in data.disp) { // por sensor
        var datapoint = [];
        for (var a in data.disp[i].values) {
//                            datapoint.push({"sensor": data.disp[i].name, "date": data.disp[i].values[a].Last_time, "power": data.disp[i].values[a].Power});
            datapoint.push({x: new Date(data.disp[i].values[a].Last_time * 1000), y: data.disp[i].values[a].Power * 1});
        }
    ...
    var chart = new CanvasJS.Chart(local,
            {
                zoomEnabled: true,
                exportEnabled: true,
                animationEnabled: true,
