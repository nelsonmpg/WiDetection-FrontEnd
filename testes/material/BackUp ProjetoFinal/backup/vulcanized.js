...
(function (scope) {
    ...
    pathTest = null;
    var target = {shadow: function (inEl) {
            if (inEl) {
...
(function () {
    ...
    var attrib2css = ["none", "auto", "pan-x", "pan-y", {rule: "pan-x pan-y", selectors: ["pan-x pan-y", "pan-y pan-x"]}, "manipulation"];
    var styles = "";
...
(function (scope) {
    ...
    var eventFactory = {preventTap: NOP_FACTORY, makeBaseEvent: function (inType, inDict) {
...
(function (scope) {
    ...
    PointerMap.prototype = {set: function (inId, inEvent) {
...
(function (scope) {
    ...
    var currentGestures;
    var dispatcher = {IS_IOS: false, pointermap: new scope.PointerMap, requiredGestures: new scope.PointerMap, eventMap: Object.create(null), eventSources: Object.create(null), eventSourceList: [], gestures: [], dependencyMap: {down: {listeners: 0, index: -1}, up: {listeners: 0, index: -1}}, gestureQueue: [], registerSource: function (name, source) {
            var s = source;
...
(function (scope) {
    ...
    var mouseEvents = {POINTER_ID: 1, POINTER_TYPE: "mouse", events: ["mousedown", "mousemove", "mouseup"], exposes: ["down", "up", "move"], register: function (target) {
        ...
        }, lastTouches: [], isEventSimulatedFromTouch: function (inEvent) {
...
(function (scope) {
    ...
    var touchEvents = {IS_IOS: false, events: ["touchstart", "touchmove", "touchend", "touchcancel"], exposes: ["down", "up", "move"], register: function (target, initial) {
        ...
        }, scrollTypes: {EMITTER: "none", XSCROLLER: "pan-x", YSCROLLER: "pan-y"}, touchActionToScrollType: function (touchAction) {
            var t = touchAction;
        ...
        }, POINTER_TYPE: "touch", firstTouch: null, isPrimaryTouch: function (inTouch) {
            ...
            if (this.currentTouchEvent.type === "touchstart") {
                if (this.isPrimaryTouch(touch)) {
                    var fastPath = {clientX: touch.clientX, clientY: touch.clientY, path: this.currentTouchEvent.path, target: this.currentTouchEvent.target};
...
(function (scope) {
    ...
    var msEvents = {events: ["MSPointerDown", "MSPointerMove", "MSPointerUp", "MSPointerCancel"], register: function (target) {
            dispatcher.listen(target, this.events)
        }, unregister: function (target) {
            if (target === document) {
                return
            ...
            dispatcher.unlisten(target, this.events)
        }, POINTER_TYPES: ["", "unavailable", "touch", "pen", "mouse"], prepareEvent: function (inEvent) {
            var e = inEvent;
...
(function (scope) {
    var dispatcher = scope.dispatcher;
    var pointermap = dispatcher.pointermap;
    var pointerEvents = {events: ["pointerdown", "pointermove", "pointerup", "pointercancel"], prepareEvent: function (inEvent) {
            var e = dispatcher.cloneEvent(inEvent);
            e._source = "pointer";
            return e
...
(function (scope) {
    ...
    var track = {events: ["down", "move", "up"], exposes: ["trackstart", "track", "trackx", "tracky", "trackend"], defaultActions: {track: "none", trackx: "pan-y", tracky: "pan-x"}, WIGGLE_THRESHOLD: 4, clampDir: function (inDelta) {
            ...
            var gestureProto = {bubbles: true, cancelable: true, trackInfo: t.trackInfo, relatedTarget: inEvent.relatedTarget, pointerType: inEvent.pointerType, pointerId: inEvent.pointerId, _source: "track"};
            if (inType !== "tracky") {
            ...
            if (inEvent.isPrimary && (inEvent.pointerType === "mouse" ? inEvent.buttons === 1 : true)) {
                var p = {downEvent: inEvent, downTarget: inEvent.target, trackInfo: {}, lastMoveEvent: null, xDirection: 0, yDirection: 0, tracking: false};
...
(function (scope) {
    var dispatcher = scope.dispatcher;
    var eventFactory = scope.eventFactory;
    var hold = {HOLD_DELAY: 200, WIGGLE_THRESHOLD: 16, events: ["down", "move", "up"], exposes: ["hold", "holdpulse", "release"], heldPointer: null, holdJob: null, pulse: function () {
            ...
            var p = {bubbles: true, cancelable: true, pointerType: this.heldPointer.pointerType, pointerId: this.heldPointer.pointerId, x: this.heldPointer.clientX, y: this.heldPointer.clientY, _source: "hold"};
            if (inHoldTime) {
...
(function (scope) {
    ...
    var eventFactory = scope.eventFactory;
    var pointermap = new scope.PointerMap;
    var tap = {events: ["down", "up"], exposes: ["tap"], down: function (inEvent) {
            if (inEvent.isPrimary && !inEvent.tapPrevented) {
                pointermap.set(inEvent.pointerId, {target: inEvent.target, buttons: inEvent.buttons, x: inEvent.clientX, y: inEvent.clientY})
            }
            ...
            if (start && this.shouldTap(inEvent, start)) {
                ...
                if (t) {
                    var e = eventFactory.makeGestureEvent("tap", {bubbles: true, cancelable: true, x: inEvent.clientX, y: inEvent.clientY, detail: inEvent.detail, pointerType: inEvent.pointerType, pointerId: inEvent.pointerId, altKey: inEvent.altKey, ctrlKey: inEvent.ctrlKey, metaKey: inEvent.metaKey, shiftKey: inEvent.shiftKey, _source: "tap"});
                    t.dispatchEvent(e)
                }
            }
            pointermap.delete(inEvent.pointerId)
...
(function (scope) {
    ...
    var pinch = {events: ["down", "up", "move", "cancel"], exposes: ["pinch", "rotate"], defaultActions: {pinch: "none", rotate: "none"}, reference: {}, down: function (inEvent) {
            pointermap.set(inEvent.pointerId, inEvent);
            if (pointermap.pointers() == 2) {
                var points = this.calcChord();
                var angle = this.calcAngle(points);
                this.reference = {angle: angle, diameter: points.diameter, target: scope.targetFinding.LCA(points.a.target, points.b.target)}
            }
        }, up: function (inEvent) {
            var p = pointermap.get(inEvent.pointerId);
            if (p) {
            ...
            var e = eventFactory.makeGestureEvent("pinch", {bubbles: true, cancelable: true, scale: zoom, centerX: points.center.x, centerY: points.center.y, _source: "pinch"});
            this.reference.target.dispatchEvent(e)
        }, fireRotate: function (angle, points) {
            var diff = Math.round((angle - this.reference.angle) % 360);
            var e = eventFactory.makeGestureEvent("rotate", {bubbles: true, cancelable: true, angle: diff, centerX: points.center.x, centerY: points.center.y, _source: "pinch"});
            this.reference.target.dispatchEvent(e)
...
(function (global) {
    ...
    Syntax = {ArrayExpression: "ArrayExpression", BinaryExpression: "BinaryExpression", CallExpression: "CallExpression", ConditionalExpression: "ConditionalExpression", EmptyStatement: "EmptyStatement", ExpressionStatement: "ExpressionStatement", Identifier: "Identifier", Literal: "Literal", LabeledStatement: "LabeledStatement", LogicalExpression: "LogicalExpression", MemberExpression: "MemberExpression", ObjectExpression: "ObjectExpression", Program: "Program", Property: "Property", ThisExpression: "ThisExpression", UnaryExpression: "UnaryExpression"};
    Messages = {UnexpectedToken: "Unexpected token %0", UnknownLabel: "Undefined label '%0'", Redeclaration: "%0 '%1' has already been declared"};
    ...
    function scanIdentifier() {
        ...
        return{type: type, value: id, range: [start, index]}
    }
    function scanPunctuator() {
        ...
        switch (code) {
            ...
            case 63:
                ++index;
                return{type: Token.Punctuator, value: String.fromCharCode(code), range: [start, index]};
            default:
                ...
                if (code2 === 61) {
                    switch (code) {
                        ...
                        case 124:
                            index += 2;
                            return{type: Token.Punctuator, value: String.fromCharCode(code) + String.fromCharCode(code2), range: [start, index]};
                        case 33:
                        case 61:
                            index += 2;
                            ...
                            return{type: Token.Punctuator, value: source.slice(start, index), range: [start, index]};
                        default:
                            break
                        }
                }
        ...
        if (ch1 === ch2 && "&|".indexOf(ch1) >= 0) {
            index += 2;
            return{type: Token.Punctuator, value: ch1 + ch2, range: [start, index]}
        }
        if ("<>=!+-*%&|^/".indexOf(ch1) >= 0) {
            ++index;
            return{type: Token.Punctuator, value: ch1, range: [start, index]}
        }
    ...
    function scanNumericLiteral() {
        ...
        return{type: Token.NumericLiteral, value: parseFloat(number), range: [start, index]}
    }
    function scanStringLiteral() {
        ...
        return{type: Token.StringLiteral, value: str, octal: octal, range: [start, index]}
    }
    ...
    function advance() {
        ...
        skipWhitespace();
        if (index >= length) {
            return{type: Token.EOF, range: [index, index]}
        }
        ch = source.charCodeAt(index);
...
(function (global) {
    ...
    Literal.prototype = {valueFn: function () {
            if (!this.valueFn_) {
    ...
    IdentPath.prototype = {valueFn: function () {
            if (!this.valueFn_) {
                var name = this.name;
                var path = this.path;
    ...
    MemberExpression.prototype = {get fullPath() {
            if (!this.fullPath_) {
    ...
    Filter.prototype = {transform: function (model, observer, filterRegistry, toModelDirection, initialArgs) {
    ...
    var binaryOperators = {"+": function (l, r) {
            return l + r
        }, "-": function (l, r) {
            return l - r
        }, "*": function (l, r) {
    ...
    ASTDelegate.prototype = {createUnaryExpression: function (op, argument) {
            ...
            return{key: key instanceof IdentPath ? key.name : key.value, value: value}
    ...
    ConstantObservable.prototype = {open: function () {
            return this.value_
    ...
    Expression.prototype = {getBinding: function (model, filterRegistry, oneTime) {
            if (oneTime)
    ...
    PolymerExpressions.prototype = {styleObject: function (value) {
            var parts = [];
    ...
    var createScopeObject = "__proto__"in{} ? function (parentScope, model, scopeName, indexName) {
        ...
        Object.defineProperty(scope, scopeName, {value: model, configurable: true, writable: true});
        Object.defineProperty(scope, indexName, {value: undefined, configurable: true, writable: true});
        Object.defineProperty(scope, parentScopeName, {value: parentScope, configurable: true, writable: true});
        return scope
    };
    global.PolymerExpressions = PolymerExpressions;
    PolymerExpressions.getExpression = getExpression
})(this);
Polymer = {version: "0.5.1"};
...
if (!window.WebComponents) {
    if (!window.WebComponents) {
        WebComponents = {flush: function () {
            }, flags: {log: {}}};
        Platform = WebComponents;
        CustomElements = {useNative: true, ready: true, takeRecords: function () {
            }, "instanceof": function (obj, base) {
                return obj instanceof base
            }};
        HTMLImports = {useNative: true};
    ...
    (function (scope) {
        ...
        var currentScriptDescriptor = {get: function () {
...
(function (global) {
    ...
    Path.prototype = createObject({__proto__: [], valid: true, toString: function () {
            var pathString = "";
    ...
    function newObservedSet() {
        ...
        var record = {objects: objects, get rootObject() {
                return rootObj
    ...
    Observer.prototype = {open: function (callback, target) {
    ...
    ObjectObserver.prototype = createObject({__proto__: Observer.prototype, arrayObserve: false, connect_: function (callback, target) {
            if (hasObserve) {
    ...
    ArrayObserver.prototype = createObject({__proto__: ObjectObserver.prototype, arrayObserve: true, copyObject: function (arr) {
            return arr.slice()
    ...
    PathObserver.prototype = createObject({__proto__: Observer.prototype, get path() {
            return this.path_
        }, connect_: function () {
            if (hasObserve)
    ...
    CompoundObserver.prototype = createObject({__proto__: Observer.prototype, connect_: function () {
            if (hasObserve) {
                var object;
    ...
    ObserverTransform.prototype = {open: function (callback, target) {
            this.callback_ = callback;
            this.target_ = target;
    ...
    var expectedRecordTypes = {add: true, update: true, "delete": true};
...
(function (global) {
    ...
    Object.defineProperty(Platform, "enableBindingsReflection", {get: function () {
...
(function (global) {
    ...
    var REPEAT = "repeat";
    var IF = "if";
    var templateAttributeDirectives = {template: true, repeat: true, bind: true, ref: true};
    var semanticTemplateElements = {THEAD: true, TBODY: true, TFOOT: true, TH: true, TR: true, TD: true, COLGROUP: true, COL: true, CAPTION: true, OPTION: true, OPTGROUP: true};
    ...
    var contentDescriptor = {get: function () {
    ...
    mixin(HTMLTemplateElement.prototype, {bind: function (name, value, oneTime) {
            if (name != "ref")
            ...
            if (templateObserver) {
                templateObserver.observe(this, {attributes: true, attributeFilter: ["ref"]})
            }
            ...
            var instanceRecord = instance.templateInstance_ = {firstNode: null, lastNode: null, model: model};
            var i = 0;
            ...
            return{bindingMaps: {}, raw: bindingDelegate, prepareBinding: delegateFn("prepareBinding"), prepareInstanceModel: delegateFn("prepareInstanceModel"), prepareInstancePositionChanged: delegateFn("prepareInstancePositionChanged")}
    ...
    Object.defineProperty(Node.prototype, "templateInstance", {get: function () {
    ...
    TemplateIterator.prototype = {closeDeps: function () {
            var deps = this.deps;
            if (deps) {
...
(function (scope) {
    var urlResolver = {resolveDom: function (root, url) {
...
(function (scope) {
    ...
    Loader.prototype = {extractUrls: function (text, base) {
            var matches = [];
            var matched, u;
...
(function (scope) {
    ...
    StyleResolver.prototype = {regex: /@import\s+(?:url)?["'\(]*([^'"\)]*)['"\)]*;/g, resolve: function (text, url, callback) {
...
(function (scope) {
    ...
    Job.prototype = {go: function (callback, wait) {
...
(function (scope) {
    ...
    var typeHandlers = {string: noopHandler, undefined: noopHandler, date: function (value) {
...
(function (scope) {
    var utils = {async: function (method, args, timeout) {
            Polymer.flush();
            ...
            var event = new CustomEvent(type, {bubbles: bubbles !== undefined ? bubbles : true, cancelable: cancelable !== undefined ? cancelable : true, detail: detail});
...
(function (scope) {
    var log = window.WebComponents ? WebComponents.flags.log : {};
    var EVENT_PREFIX = "on-";
    var events = {EVENT_PREFIX: EVENT_PREFIX, addHostListeners: function () {
...
(function (scope) {
    var attributes = {copyInstanceAttributes: function () {
...
(function (scope) {
    ...
    var empty = [];
    var updateRecord = {object: undefined, type: "update", name: undefined, oldValue: undefined};
    ...
    var properties = {createPropertyObserver: function () {
...
(function (scope) {
    var log = window.WebComponents ? WebComponents.flags.log : {};
    var mdv = {instanceTemplate: function (template) {
...
(function (scope) {
    var base = {PolymerBase: true, job: function (job, callback, wait) {
...
(function (scope) {
    ...
    var styles = {STYLE_SCOPE_ATTRIBUTE: STYLE_SCOPE_ATTRIBUTE, installControllerStyles: function () {
...
(function (scope) {
    var path = {resolveElementPaths: function (node) {
...
(function (scope) {
    ...
    var styles = {loadStyles: function (callback) {
...
(function (scope) {
    ...
    var events = {parseHostEvents: function () {
            ...
            return function (model, node, oneTime) {
                ...
                return{open: bindingValue, discardChanges: bindingValue, close: function () {
...
(function (scope) {
    ...
    var properties = {inferObservers: function (prototype) {
...
(function (scope) {
    ...
    var attributes = {inheritAttributesObjects: function (prototype) {
...
(function (scope) {
    ...
    var mdv = {syntax: syntax, fetchTemplate: function () {
...
(function (scope) {
    ...
    var extend = scope.extend;
    var hasShadowDOMPolyfill = window.ShadowDOMPolyfill;
    var prototype = {register: function (name, extendeeName) {
            ...
            var info = {prototype: this.prototype};
...
(function (scope) {
    var queue = {wait: function (element) {
            if (!element.__queue) {
                element.__queue = {};
...
(function (scope) {
    ...
    var prototype = extend(Object.create(HTMLElement.prototype), {createdCallback: function () {
...
(function () {
    ...
    element.init();
    Polymer("auto-binding", {createdCallback: function () {
...
Polymer.CoreFocusable = {mixinPublish: {active: {value: false, reflect: true}, focused: {value: false, reflect: true}, pressed: {value: false, reflect: true}, disabled: {value: false, reflect: true}, toggle: false}, mixinDelegates: {contextMenu: "_contextMenuAction", down: "_downAction", up: "_upAction", focus: "_focusAction", blur: "_blurAction"}, mixinObserve: {disabled: "_disabledChanged"}, _disabledChanged: function () {
        if (this.disabled) {
...
(function (a) {
    ...
    var b = document.location, d = {mode: "modern", hash: b.hash, history: !1, check: function () {
        ...
        if (this instanceof e)
            this.params = {}, this.routes = {}, this.methods = ["on", "once", "after", "before"], this.scope = [], this._methods = {}, this._insert = this.insert, this.insert = this.insertEx, this.historySupport = (window.history != null ? window.history.pushState : null) != null, this.configure(), this.mount(a || {});
        else
            return new e(a)
    };
    e.prototype.init = function (a) {
        ...
        if (this.history === !1)
            c() && a ? b.hash = a : c() || e.dispatch("on", "/" + b.hash.replace(/^(#\/|#|\/)/, ""));
        else {
...
(function () {
    ...
    Polymer("core-style", {publish: {ref: ""}, g: CoreStyle.g, refMap: CoreStyle.refMap, list: CoreStyle.list, ready: function () {
            if (this.id) {
                this.provide()
            } else {
            ...
            this.domObserver = new MutationObserver(this.domModified.bind(this)).observe(this.shadowRoot, {subtree: true, characterData: true, childList: true});
            this.provideContent()
...
Polymer("core-transition", {type: "transition", go: function (node, state) {
        this.complete(node)
    }, setup: function (node) {
...
(function () {
    ...
    Polymer("core-transition-pages", {publish: {scopeClass: "", activeClass: "", transitionProperty: ""}, completed: false, prepare: function (scope, options) {
...
(function () {
    ...
    var hasShadowDOMPolyfill = window.ShadowDOMPolyfill;
    Polymer("hero-transition", {go: function (scope, options) {
            ...
            for (var i = 0, h0; h0 = h$[i]; i++) {
                ...
                if (v && h1) {
                    ...
                    var h = {h0: h0, b0: h0.getBoundingClientRect(), r0: c0.borderRadius, h1: h1, b1: h1.getBoundingClientRect(), r1: c1.borderRadius};
...
Polymer("core-animated-pages", {eventDelegates: {"core-transitionend": "transitionEnd"}, transitions: "", selected: 0, lastSelected: null, registerCallback: function () {
        ...
        var options = {src: src, dst: dst, easing: "cubic-bezier(0.4, 0, 0.2, 1)"};
...
(function () {
    Polymer("core-toolbar", {justify: "", middleJustify: "", bottomJustify: "", justifyChanged: function (old) {
...
Polymer("core-media-query", {queryMatches: false, query: "", ready: function () {
...
Polymer("core-header-panel", {publish: {mode: {value: "", reflect: true}, tallClass: "tall", shadow: false}, animateDuration: 200, modeConfigs: {shadowMode: {waterfall: 1, "waterfall-tall": 1}, noShadow: {seamed: 1, cover: 1, scroll: 1}, tallMode: {"waterfall-tall": 1}, outerScroll: {scroll: 1}}, ready: function () {
...
(function () {
    var meta;
    Polymer("core-icon", {src: "", icon: "", alt: null, observe: {icon: "updateIcon", alt: "updateAlt"}, defaultIconset: "icons", ready: function () {
            if (!meta) {
...
Polymer("core-scaffold", {publish: {drawerWidth: "256px", responsiveWidth: "600px", rightDrawer: false, disableSwipe: false, mode: {value: "seamed", reflect: true}}, ready: function () {
...
Polymer("core-menu");
(function () {
    var p = {eventDelegates: {down: "downAction"}, activeChanged: function () {
            this.super();
            if (this.$.ripple) {
                if (this.active) {
                    if (!this.lastEvent) {
...
(function () {
    ...
    function createWave(elem) {
        ...
        var wave = {bg: elem.$.bg, wc: outer, wave: inner, waveColor: fgColor, maxRadius: 0, isMouseDown: false, mouseDownStart: 0, mouseUpStart: 0, tDown: 0, tUp: 0};
        return wave
    }
    ...
    Polymer("paper-ripple", {initialOpacity: .25, opacityDecayVelocity: .8, backgroundFill: true, pixelDensity: 2, eventDelegates: {down: "downAction", up: "upAction"}, ready: function () {
            this.waves = []
        }, downAction: function (e) {
            ...
            var anim = {initialOpacity: this.initialOpacity, opacityDecayVelocity: this.opacityDecayVelocity, height: ctx.height, width: ctx.width};
            for (var i = 0; i < this.waves.length; i++) {
                var wave = this.waves[i];
...
Polymer("paper-item", {publish: {raised: false, recenteringTouch: false, fill: true}});
(function () {
    var private_router;
    Polymer("flatiron-director", {autoHash: false, ready: function () {
            this.router.on(/(.*)/, function (route) {
                this.route = route
            }.bind(this));
            this.route = this.router.getRoute() ? this.router.getRoute().join(this.router.delimiter) : ""
...
(function () {
    ...
    function keyboardEventToKey(ev) {
        ...
        return{shift: ev.shiftKey, ctrl: ev.ctrlKey, meta: ev.metaKey, alt: ev.altKey, key: normalizedKey}
    }
    ...
    Polymer("core-a11y-keys", {created: function () {
        ...
        }, publish: {keys: "", target: null}, keysChanged: function () {
...
Polymer("core-ajax", {url: "", handleAs: "", auto: false, params: "", response: null, error: null, loading: false, progress: null, method: "", headers: null, body: null, contentType: "application/x-www-form-urlencoded", withCredentials: false, xhrArgs: null, created: function () {
        this.progress = {}
    }, ready: function () {
        ...
        var progressProxy = {lengthComputable: progress.lengthComputable, loaded: progress.loaded, total: progress.total};
        ...
        if (this.activeRequest) {
            ...
            if ("onprogress"in activeRequest) {
                this.activeRequest.addEventListener("progress", function (progress) {
                    this.processProgress(progress, activeRequest)
                }.bind(this), false)
            } else {
                this.progress = {lengthComputable: false}
            }
        }
