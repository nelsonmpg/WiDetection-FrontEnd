            ...
            if (line[2] == ":" && line.length > 4) {
                ...
                if (result.length < 7) {
                    r.db(dbConfig.db).table("cliente")
                            .filter({"macCliente": result[0]}).update({
                        "macCliente": result[0],
                        "disp": [{
                                name: clienteSend,
                                "antena": [{
                    ...
                    r.db(dbConfig.db).table("ap").filter({"BSSID": result[0]}).update({
                        "BSSID": result[0],
                        "disp": [{
                                name: clienteSend,
                                "antena": [{
                                        "First_time_seen": result[1],
                                        "Last_time_seen": result[2],
                                        "channel": result[3],
                                        "Speed": result[4],
                                        "Privacy": result[5],
                                        "Cipher": result[6].split(" ")[0],
                                        "Authentication": result[6].split(" ")[1],
